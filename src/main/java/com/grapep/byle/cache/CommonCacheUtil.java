...
public class CommonCacheUtil {
     ...
     * @throws ByleException
     */
    public int cacheForVerificationCode(String key, String value, String type, int timeout, String ip) throws ByleException {
        try {
            JedisPool pool = jedisPoolWrapper.getJedisPool();
            if (pool != null) {
                try (Jedis jedis = pool.getResource()) {
                    ...
                    try {
                        // trans.set(key, value);
                        jedis.expire(key, timeout); //set key  timeout
                        long val = jedis.incr(key + "." + type);  //增加手机号登录的次数
                        if (val == 1) {
                            jedis.expire(key + "." + type, 86400);
                        }

                        jedis.incr(ipKey);   //增加ip登录的次数
                        if (val == 1) {
            ...
            throw new ByleException("Fail to cache for expiry");
        }
     ...
     * @throws ByleException
     */
    public UserElement getUserByToken(String token) throws ByleException {
        UserElement ue = null;
        ...
        if (pool != null) {
            try (Jedis jedis = pool.getResource()) {
                ...
                try {
                    ...
                    throw new ByleException("Fail to get token content");
                }
            }
        }
        pool.close();
